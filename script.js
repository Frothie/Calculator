      ...
      switch(btnValue) {
        ...
        case '=':
          ...
          if(operators.includes(lastChar) || lastChar== '.'){
            ...
            // if the argument is one or more JavaScript statements, eval() will execute the statements.
            inputScreen.innerHTML = eval(input);
          }
          decimalAdded= false; //reseting this
          break;
        case '.':
          if(!decimalAdded) {
        ...
        case '-':
          //Last char of string
          var lastChar = input[input.length - 1];
          //Only add operator if input is not empty and there is no operator at the last character
          if(input != ''&& !operators.includes(lastChar))
            inputScreen.innerHTML += btnValue;
          //Allows for minus if the string is empty. The first number could be under zero
          else{
            if(input == ''&& btnValue== '-')
            { inpuScreen.innerHTML += btnValue; }
          }
